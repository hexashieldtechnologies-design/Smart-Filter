...
export default function SecurityScreen() {
  ...
  const handleBiometricToggle = async () => {
    if (!biometricEnabled && secureVaultNative.available) {
      const capability = await secureVaultNative.getDeviceSecurityStatus();
      if (!capability.canAuthenticate) {
        Alert.alert('Device lock required', 'Set up a fingerprint, face unlock, or device PIN in Android Settings before enabling Secure Vault lock.');
        return;
      }
    }
    if (secureVaultNative.available) {
      const authenticated = await secureVaultNative.authenticate('Authenticate to change Secure Vault lock settings');
